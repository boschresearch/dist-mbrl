...
def truncated_normal_init(m: nn.Module):
    ...
    if isinstance(m, nn.Linear):
        ...
        nn.init.trunc_normal_(m.weight.data, std=stddev, a=-2 * stddev, b=2 * stddev)
    ...
    if isinstance(m, EnsembleLinearLayer):
        ...
        for i in range(num_members):
            nn.init.trunc_normal_(
                m.weight.data[i], std=stddev, a=-2 * stddev, b=2 * stddev
            )
        m.bias.data.fill_(0.0)
...
def to_tensor(x: dist_mbrl.mbrl.types.TensorType):
