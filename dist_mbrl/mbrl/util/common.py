...
def create_one_dim_tr_model(
    ...
    if model_cfg._target_ == "dist_mbrl.mbrl.models.BasicEnsemble":
    ...
    if name_obs_process_fn:
        ...
        obs_process_fn = None
    dynamics_model = dist_mbrl.mbrl.models.OneDTransitionRewardModel(
        model,
...
def train_model_and_save_model_and_data(
    model: dist_mbrl.mbrl.models.Model,
    model_trainer: dist_mbrl.mbrl.models.ModelTrainer,
    cfg: omegaconf.DictConfig,
    replay_buffer: ReplayBuffer,
    ...
    """
    dataset_train, dataset_val = dist_mbrl.mbrl.util.common.get_basic_buffer_iterators(
        replay_buffer,
...
def rollout_model_env(
    model_env: dist_mbrl.mbrl.models.ModelEnv,
    initial_obs: np.ndarray,
    plan: Optional[np.ndarray] = None,
    agent: Optional[dist_mbrl.mbrl.planning.Agent] = None,
    num_samples: int = 1,
    ...
    while True:
        ...
        while not terminated:
            if replay_buffer is not None:
                next_obs, reward, terminated, *_ = step_env_and_add_to_buffer(
                    env,
                    obs,
                    agent,
                    agent_kwargs,
                ...
                if callback:
                    callback((obs, action, next_obs, reward, terminated))
            obs = next_obs
            total_reward += reward
            step += 1
