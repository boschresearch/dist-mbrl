...
class NormalGammaPosterior(Posterior):
    """
    Class representing a Normal-Gamma model of a random variable with unknown mean and
    variance. Prior parameters are specified for a multi-dimensional Normal-Gamma with
    independent dimensions. Each dimension has a set of parameters [mu, kappa, alpha,
    beta].
    ...
    def update_params(self, counts, x_sum, xsquared_sums):
        ...
        beta_n = beta + 0.5 * n_sa * (
            var + kappa * (first_moment - mu) ** 2 / (kappa + n_sa)
        )
...
class NormalPosterior(Posterior):
    """
    Distribution specified by two parameters [mu, tau] representing the prior mean and
    precision of a normal distribution
    """

    def __init__(self, prior_params: np.ndarray, tau_x=1e6):
        """
        tau_x is the known precision of the random variable x. By default we assume a
        high value, corresponding to a random variable with very low variance.
        """
        self.tau_x = tau_x
        super().__init__(prior_params)
